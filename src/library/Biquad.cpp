...
namespace c_audio_playground {
template <typename T> void BiquadCoeffT<T>::calc() {
  sample_t norm;
...
//     { \
//         float32_t data[4] = {sign float32_t(filters[0].arg), sign
//         float32_t(filters[1].arg), sign float32_t(filters[2].arg), sign
//         float32_t(filters[3].arg)}; \
//         arg = vld1q_f32(data); \
//     }
